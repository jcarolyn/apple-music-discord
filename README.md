...
Uses the Windows Media Session API to detect the currently playing track from Apple Music in your browser (Edge, Chrome, or Firefox) and display it as Discord Rich Presence, complete with album art and artist info. On startup, you pick which media source to monitor. If you choose a specific browser, only media from that browser is tracked. If you choose all sources, the app may inadvertently display non-Apple Music tracks.

For best results, play Apple Music in a single browser and select that browser as your source on startup.
...
- **Source selection**: On startup, pick which browser or media source to monitor
...
On startup, the app detects active media sources and asks you to pick one (e.g. Microsoft Edge, Google Chrome, Firefox). You can also choose to monitor all sources or exit. Press `Ctrl+C` to stop.
...
- **The elapsed timer is not the actual song position.** It counts from when the track was first detected. Seeking, rewinding, or restarting does not affect the timer. Browser media sessions do not reliably report position data.
